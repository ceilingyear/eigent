...
const VITE_PUBLIC = VITE_DEV_SERVER_URL
  ...
  : RENDERER_DIST;
const WINDOWS_APP_USER_MODEL_ID = 'com.eigent.app';
const APP_ICON_PATH =
  process.platform === 'win32'
    ...
    : app.isPackaged
      ...
      : path.join(MAIN_DIST, 'build', 'icon.png');
...
// Must match electron-builder appId so Windows taskbar/notifications use the packaged app identity.
...
function registerIpcHandlers() {
  ...
  ipcMain.handle('export-log', async () => {
    try {
      ...
      const defaultFileName = `ATAI-${appVersion}-${systemVersion}-${Date.now()}.log`;
