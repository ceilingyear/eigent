...
Config (~/.eigent/.env):
    ...
    DISABLE_STEP_SYNC=false
    SYNC_DECOMPOSE_TEXT=false
"""
...
# Batch config for decompose_text events. Disabled by default because the UI
# receives this stream directly over SSE; syncing every token adds many writes.
BATCH_WORD_THRESHOLD = 80
BATCH_FLUSH_INTERVAL_SECONDS = 2.0
...
@lru_cache(maxsize=1)
def _get_config():
    if _env_flag("DISABLE_STEP_SYNC"):
        logger.info("Step sync disabled by DISABLE_STEP_SYNC")
        return None
    ...
    return f"{server_url}/v1/chat/steps"


def _env_flag(key: str, default: bool = False) -> bool:
    value = env(key, "true" if default else "false")
...
def _try_sync(args, value, sync_url):
    ...
    if step == "decompose_text":
        if not _env_flag("SYNC_DECOMPOSE_TEXT"):
            return
...
def _should_flush(task_id: str) -> bool:
    ...
    now = time.monotonic()
    last_flush = _last_flush_at.get(task_id, 0.0)
    return (
        word_count >= BATCH_WORD_THRESHOLD
        or now - last_flush >= BATCH_FLUSH_INTERVAL_SECONDS
    )
...
def _flush_buffer(task_id: str, sync_url: str):
    ...
    _last_flush_at[task_id] = time.monotonic()
