...
export const useInstallationSetup = () => {
  ...
  const getBackendHealthUrl = useCallback(async () => {
  ...
  const startBackendPolling = useCallback(() => {
    ...
    const checkBackendStatus = async () => {
      try {
        const healthUrl = await getBackendHealthUrl();
        if (healthUrl) {
          console.log(
            '[useInstallationSetup] Checking backend health:',
            healthUrl
          );
    ...
    checkBackendStatus().then((isReady) => {
      ...
      const pollInterval = setInterval(async () => {
        try {
          ...
          if (healthUrl) {
            console.log(
              '[useInstallationSetup] Polling backend health:',
              healthUrl
            );
  ...
  }, [getBackendHealthUrl, setSuccess, setInitState, setNeedsBackendRestart]);
