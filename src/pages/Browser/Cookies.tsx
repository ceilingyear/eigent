...
export default function Cookies() {
            ...
            if (!statusResponse || !statusResponse.is_open) {
              ...
              if (newResponse && newResponse.success) {
                ...
                if (newCookieCount > currentCookieCount) {
                  ...
                  toast.success(`已添加 ${addedCount} 个 Cookie`);
  ...
  const handleLoadCookies = async () => {
    ...
    try {
      ...
      toast.error(error?.message || '加载 Cookie 失败');
      setCookieDomains([]);
    } finally {
  ...
  const handleDeleteMainDomain = async (
    ...
    try {
      ...
      toast.success(`已删除 ${mainDomain} 及其所有子域名的 Cookie`);
      ...
      toast.error(
        error?.message || `删除 ${mainDomain} 的 Cookie 失败`
      );
    } finally {
  ...
  const handleDeleteAll = async () => {
    ...
    try {
      ...
      toast.success('已删除所有 Cookie');
      setCookieDomains([]);

      setHasUnsavedChanges(true);
      setShowRestartDialog(true);
    } catch (error: any) {
      toast.error(error?.message || '删除所有 Cookie 失败');
    } finally {
  ...
  return (
    <div className="m-auto flex h-auto w-full flex-1 flex-col">
      <AlertDialog
        ...
        title="Cookie 已更新"
        message="Cookie 已更新。是否重启应用以使用新的 Cookie？"
        confirmText="重启"
        cancelText="暂不重启"
